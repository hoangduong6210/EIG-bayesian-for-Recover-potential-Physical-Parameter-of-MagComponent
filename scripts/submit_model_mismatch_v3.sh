#!/usr/bin/env bash
# Submit the independent MM-3 successor from a prepared immutable run.

set -Eeuo pipefail
umask 027

if [[ $# -ne 3 ]]; then
    echo "usage: $0 PREPARED_RUN_DIR ESTIMATOR_DECISION_JSON MM2_NON_ADMISSION_JSON" >&2
    exit 64
fi
...
case "$RUN_DIR/" in
    /tmp/*|/var/tmp/*)
        [[ "${MAGCORE_TEST_ALLOW_EPHEMERAL_RUN_ROOT:-0}" == "1" ]] || {
            echo "MM-3 run must be on a compute-node shared filesystem" >&2
            exit 65
        }
        ;;
esac
...
[[ -s "$PREDECESSOR" ]] || { echo "missing MM-2 non-admission record: $PREDECESSOR" >&2; exit 66; }
for command in sbatch sha256sum tar diff mktemp; do
    command -v "$command" >/dev/null || { echo "missing command: $command" >&2; exit 69; }
done
for script in 32_model_mismatch_v3.sbatch 33_model_mismatch_v3_aggregate.sbatch; do
...
[[ -f "$RUN_DIR/provenance/git-status.txt" && ! -s "$RUN_DIR/provenance/git-status.txt" \
    && "$MAGCORE_SOURCE_STATUS_SHA256" == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" ]] || {
    echo "MM-3 requires a clean prepared source revision" >&2
    exit 68
}
...
CONFIG="$RUN_DIR/source/configs/model_mismatch_v3.toml"
[[ -s "$CONFIG" ]] || { echo "prepared source does not contain MM-3" >&2; exit 66; }
# Resolve the loader from the verified source, independently of editable installs.
export PYTHONPATH="$RUN_DIR/source/src"
export PYTHONDONTWRITEBYTECODE=1
PLAN="$($MAGCORE_VENV/bin/python \
    "$RUN_DIR/source/scripts/model_mismatch_plan.py" plan --config "$CONFIG")"
printf '%s' "$PLAN" | "$MAGCORE_VENV/bin/python" -c '
import json, sys
p = json.load(sys.stdin)
if p.get("campaign_id") != "MM-3" or p.get("schema_version") != "magcore-model-mismatch-preregistration/1.2" or p.get("task_count") != 120:
    raise SystemExit("prepared plan is not the 120-task MM-3 preregistration")
if p.get("runtime", {}).get("sampler_method") != "de_snooker" or p.get("lineage", {}).get("predecessor_campaign_id") != "MM-2":
    raise SystemExit("prepared plan has incompatible sampler or predecessor")
'
...
record_submission_failure() {
    ...
    printf '{"stage":"%s","array_job_id":"%s","aggregate_job_id":"%s","exit_status":%s}\n' \
        ...
        > "$RUN_DIR/status/MM3_SUBMISSION_FAILED.partial"
    mv "$RUN_DIR/status/MM3_SUBMISSION_FAILED.partial" \
        "$RUN_DIR/status/MM3_SUBMISSION_FAILED"
}

DESTINATION="$RUN_DIR/summary/model_mismatch_v3"
mkdir -p "$DESTINATION"
for marker in MM3_SUBMISSION_LOCK MM3_SUBMISSION_FAILED MM3_ARRAY_SUBMITTED MM3_JOBS_ACCEPTED MM3_SUBMITTED; do
    [[ ! -e "$RUN_DIR/status/$marker" ]] || {
        echo "refusing to resubmit MM-3 with an existing scheduler marker: $marker" >&2
        exit 73
    }
done
for destination in estimator_decision.json predecessor_non_admission.json; do
    [[ ! -e "$DESTINATION/$destination" ]] || {
        echo "refusing to replace locked MM-3 input: $destination" >&2
        exit 73
    }
done
# The persistent atomic lock also prevents concurrent or interrupted resubmission.
mkdir "$RUN_DIR/status/MM3_SUBMISSION_LOCK" || {
    echo "refusing concurrent or repeated MM-3 submission" >&2
    exit 73
}
trap 'STATUS=$?; record_submission_failure interrupted "$STATUS" "${ARRAY:-}" "${AGGREGATE:-}"' ERR
...
if ARRAY="$(submit "" 32_model_mismatch_v3.sbatch --array="0-$((TASK_COUNT - 1))%10")"; then
    :
else
    STATUS="$?"; record_submission_failure array "$STATUS" "" ""; exit "$STATUS"
fi
printf '{"array_job_id":"%s","task_count":%s}\n' "$ARRAY" "$TASK_COUNT" \
    > "$RUN_DIR/status/MM3_ARRAY_SUBMITTED.partial"
mv "$RUN_DIR/status/MM3_ARRAY_SUBMITTED.partial" "$RUN_DIR/status/MM3_ARRAY_SUBMITTED"
if AGGREGATE="$(submit "afterok:$ARRAY" 33_model_mismatch_v3_aggregate.sbatch)"; then
    :
else
    STATUS="$?"; record_submission_failure aggregate "$STATUS" "$ARRAY" ""; exit "$STATUS"
fi
printf '{"array_job_id":"%s","aggregate_job_id":"%s","task_count":%s}\n' \
    "$ARRAY" "$AGGREGATE" "$TASK_COUNT" > "$RUN_DIR/status/MM3_JOBS_ACCEPTED.partial"
mv "$RUN_DIR/status/MM3_JOBS_ACCEPTED.partial" "$RUN_DIR/status/MM3_JOBS_ACCEPTED"
{
    printf 'stage\tjob_id\tdependency\ttask_count\n'
    printf 'model_mismatch_v3\t%s\t-\t%s\n' "$ARRAY" "$TASK_COUNT"
    printf 'model_mismatch_v3_aggregate\t%s\tafterok:%s\t1\n' "$AGGREGATE" "$ARRAY"
} > "$RUN_DIR/provenance/model_mismatch_v3_jobs.tsv.partial"
mv "$RUN_DIR/provenance/model_mismatch_v3_jobs.tsv.partial" \
    "$RUN_DIR/provenance/model_mismatch_v3_jobs.tsv"
printf '{"array_job_id":"%s","aggregate_job_id":"%s","task_count":%s}\n' \
    "$ARRAY" "$AGGREGATE" "$TASK_COUNT" > "$RUN_DIR/status/MM3_SUBMITTED.partial"
mv "$RUN_DIR/status/MM3_SUBMITTED.partial" "$RUN_DIR/status/MM3_SUBMITTED"
printf 'Submitted MM-3 array: %s (%s tasks)\nAggregate job: %s\nRun: %s\n' \
