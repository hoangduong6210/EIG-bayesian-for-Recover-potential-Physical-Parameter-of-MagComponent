...
- The final policy states include a disjoint 23-point latent holdout and
  six-parameter recovery records.
...
The scientific evidence freeze is valid. The previous pipeline's publication
stage failed after a PDF build because of a bibliography packaging-path bug;
the reproducibility fix is committed. Under the new workflow this wiki is the
living manuscript, so no replacement paper is published automatically. A
sanitized audit asset and an explicit wiki-derived paper snapshot remain
release tasks.
